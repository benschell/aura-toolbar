...
export default {
  ...
  activate(state) {
    this.bottomView = new AuraFooterView(this);
    ...
    this.bottomPanel = atom.workspace.addBottomPanel({item: this.bottomView, visible: false});
  ...
  serialize() {
    return {
      bottomView: this.bottomView.serialize()
    };
  },
  ...
  _didChangeActivePaneItem(paneItem) {
    ...
    if (currentType) {
      ...
      this.bottomView.highlight(currentType);
      ...
      ['.cmp', '.app', '.evt', '.lib', '.css', 'Controller.js', 'Helper.js', 'Renderer.js', 'Test.js'].forEach(function(type) {
        ...
        if (newPath) {
          ...
          this.bottomView.show(type);
        } else {
          // Make sure this tab is hidden
          this.bottomView.hide(type);
        }
      }.bind(this));
      if (markupWasFound) {
        this.bottomView.show('Markup');
      } else {
        this.bottomView.hide('Markup');
      }
    } else {
      this.bottomPanel.hide();
    }
